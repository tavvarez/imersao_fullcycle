...
func TestBuyPartialAsset(t *testing.T) {
	...
	assert.Equal(2, len(book.Transaction), "Should have 2 transactions")
	assert.Equal(15.0, float64(book.Transaction[0].Total), "Transaction should have price 15")
	assert.Equal(10.0, float64(book.Transaction[1].Total), "Transaction should have price 10")
}
